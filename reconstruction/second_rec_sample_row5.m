...
load('G:\IGSNRR\中国东部数据集重建\code\旱涝等级\重建\fourth_supplement_treering_in_misslocation\data\tree_row5.mat')
tree(5,4).tree=cat(2,tree(5,4).tree,row54);

val=[1951-960+1:1980-960+1];
cal=[1921-960+1:1950-960+1];
%============================reconstruction
for ii=5%1:size(DWI,1)
    for jj=1:size(DWI,2)
        tic
...
%                 stationpre=cat(2,DWI(ii,jj).DWI,tree(ii,jj).tree);
%             else
%                 stationpre=cat(2,DWI(ii,jj).DWI,tree(ii,jj).tree(:,[1,5:9]));
%             end
