...
load('./data/recsample_new/recsample12.mat');recsample(2,1)=struct('sample',sample);clear sample
load('./data/recsample_new/recsample13.mat');recsample(3,1)=struct('sample',sample);clear sample
load('./data/recsample_new/recsample14.mat');recsample(4,1)=struct('sample',sample);clear sample
load('./data/recsample_new/recsample15.mat');recsample(5,1)=struct('sample',sample);clear sample
load('./data/recsample_new/recsample16.mat');recsample(6,1)=struct('sample',sample);clear sample
load('./data/recsample_new/recsample17.mat');recsample(7,1)=struct('sample',sample);clear sample
load('./data/recsample_new/recsample18.mat');recsample(8,1)=struct('sample',sample);clear sample
load('./data/recsample_new/recsample19.mat');recsample(9,1)=struct('sample',sample);clear sample
...
% load('G:\IGSNRR\中国东部数据集重建\code\旱涝等级\重建\fourth_supplement_treering_in_misslocation\data\tree_row1.mat')
% tree(1,7).tree=cat(2,tree(1,7).tree,row17);
load('G:\IGSNRR\中国东部数据集重建\code\旱涝等级\重建\fourth_supplement_treering_in_misslocation\data\speleothems_row1.mat')
% tree(1,3).tree=cat(2,tree(1,3).tree,row13);
tree(1,4).tree=cat(2,tree(1,4).tree,row14);
        ...
        for kk1=1:size(sample,1)
            for kk2=2:size(sample,2)
                ...
                for kk3=1:size(sample(kk1,kk2).sample,1)
                    ...
                    sstotal=sum((yDWI-mean(yDWI)).^2);
                    
                    for iiii=1:length(xtree)
                        if iiii==1 
                            iiijjj=[2:length(yDWI)]';
                        elseif iiii==length(yDWI)
                            iiijjj=[1:length(yDWI)-1]';
                        elseif iiii~=1 & iiii~=length(yDWI)
                            iiijjj=[1:iiii-1,iiii+1:length(yDWI)]';
                        end
                        
                        po=regress(yDWI(iiijjj,1),[ones(length(yDWI(iiijjj,1)),1),xtree(iiijjj,:)]);
                        yDWIX2(iiii,1)=sum(repmat(po(2:end,1)',[size(xtree(iiii,:),1),1]).*xtree(iiii,:),2)+po(1,1);
                        
                    end;clear iiii
                    press=sum((yDWI-yDWIX2).^2);

                    r2(kk1,kk2).r2(kk3,1)=abs(1-press/sstotal);
                    
                    clear xtree yDWI po xtreeX yDWIX1 yDWIX2 k p sstotal press 
...
save('./data/sub_spe/zheng_subchina_row1_R2p.mat','subchina','subR2','recRECE','submiss','R2max');
