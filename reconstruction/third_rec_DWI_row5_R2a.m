...
%============================
zz=recsample(7,1).sample(:,1);
for ii=1:size(zz,1)
    zzz=zz(ii,1).sample;
    ZZ=zzz+960-1;
    
    yy(ii,1)=struct('yy',ZZ);
    clear zzz ZZ
end;clear ii zz

sss=5;jj=7;
%============================scientific data find function
load('./data/sub_spe/subchina_row5.mat');R2(5,1)=struct('R2',R2max);
dd1=R2(sss).R2(7).rmax;
dd2=recsample(7).sample;
stationpre=cat(2,DWI(sss,jj).DWI,tree(sss,jj).tree);
recon_stationpre=stationpre(recon,:);
for ii=1:size(dd1,1)
    DD=(dd2(ii,dd1(ii,1)).sample(dd1(ii,2),:))';
    
    xtree=stationpre(recon,DD);
    yDWI=squeeze(china(sss,jj,recon));
    po=regress(yDWI,[ones(length(yDWI),1),xtree]);
    
    DD=cat(1,0,DD);
    DDD=cat(2,po,DD);
    func(ii,1)=struct('func',DDD);
    
    clear DD xtree yDWI po DDD
end;clear ii
...
save('./data/sub_spe/zheng_subchina_row5_R2a.mat','subchina','subR2','recRECE','submiss','R2max');
